...
$(function() {
    //把三个类型得input分别获取
    var $theadInput = $('thead input[type=checkbox]'); //表头选中
    var $tbodyInputs = $('tbody input[type=checkbox]'); //表格中得每一行选择框
    var $totalPriceInput = $('.totalPrice input[type=checkbox]'); //计算总价中得全选框
    /* 全选*/
    /*
    1.给表头中得全选按钮 绑定点击事件 点击的时候 获取它的状态（true/false）
    2.给表格中没一行得数据得input选择框，赋值为表头得选中状态(true/false)
    3.给计算总价中得全选框，也赋值为表头得选中状态(true/false)
    */
    //表头的全选
    $theadInput.change(function() {
        var checkState = $(this).prop('checked'); //获取全选框得选中状态
    ...
    /*
    计算总价的全选
    1.给计算总价的全选按钮 绑定点击事件，获取选中状态(true/false)
    2.把状态给表头的全选
    3.把状态给表格的选择框
    */
    $totalPriceInput.change(function() {
            ...
            $theadInput.prop('checked', checkState); //赋值给表头的全选状态
            $tbodyInputs.prop('checked', checkState); //赋值给表格中全选状态
            allTotal(); //总计
        })
        /*
        表格中的选择框 反过来影响两个全选框
        ...
        5.把falg的值赋值给两个全选框(因为flag就是对应的选中状态)
        */
    $tbodyInputs.change(function() { //给表格总单选框绑事件
            var flag = true; //定标竿true
    ...
    /*
        ...
        1.给-绑定事件
        2.获取前面输入框的值
        3.值自减，边界判断，如果小于1，那么就等于1否则就等于自己
    ...
    */
    ...
    /*
      总计功能实现：（头部全选 计算总价全选 表格的选择框 + - 删除 六个地方调用总和）
      1.定义一个变量 用于保持总和 定义一个变量 用于保存已选商品 数量
